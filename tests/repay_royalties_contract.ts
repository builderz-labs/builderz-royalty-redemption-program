...
describe("repay_royalties_contract", () => {
  ...
  it("Is initialized!", async () => {
    ...
    const [nftStateAddress] = PublicKey.findProgramAddressSync(
      // @ts-ignore
      [Buffer.from("nft-state"), testNFT.mint.address.toBuffer()],
      PROGRAM_ID
    )
    ...
    tx.add(createRepayRoyaltiesInstruction({
      ...
      // @ts-ignore
      nftMint: testNFT.mint.address,
      nftMintMetadata: testNFT.metadataAddress,
      user: kp.publicKey,
      anchorRemainingAccounts: remainingAccounts,
    }, {
      latestSaleLamports: 0.001 * LAMPORTS_PER_SOL
    }))
